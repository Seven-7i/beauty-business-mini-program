...
} from "@/pages-beauty/features/beauty-project/quick-add-inventory-handoff";
import InventoryItemCreatePage from "@/pages-beauty/features/inventory/components/InventoryItemCreatePage.vue";
import {
...
const quickAddRequestId = shallowRef("");

/** 页面参数只决定保存后的回传方式，不进入库存业务数据。 */
onLoad((query) => {
  quickAddRequestId.value = resolveInventoryCreateQuickAddRequestId(
    query,
...
function completeCreate(inventoryItemId: string): void {
  completeInventoryCreateNavigation(inventoryItemId, quickAddRequestId.value, {
    getPageCount: () => getCurrentPages().length,
    showSavedToast: () =>
      uni.showToast({ title: "库存物品已保存", icon: "success" }),
    navigateBack: () => uni.navigateBack(),
    relaunchInventory: () =>
      uni.reLaunch({ url: "/pages-beauty/inventory/index" }),
  });
}
</script>
