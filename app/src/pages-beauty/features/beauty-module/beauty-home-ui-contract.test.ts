...
describe("美容模块首页定稿契约", () => {
  it("模块页签切换时同步原生标题栏标题与画布底色", () => {
    ...
    const page = readSource("../../beauty/index.vue");

    expect(pages).toMatch(
      /"path": "beauty\/index"[\s\S]*?"navigationBarBackgroundColor": "#FFF8FA"/,
    );
  ...
  it("单条近期预约进入详情，查看全部进入预约执行列表", () => {
    ...
    expect(page).toContain(
      "`/pages-beauty/appointment-detail/index?appointmentId=${encodeURIComponent(appointmentId)}`",
    );
  ...
  it("业务入口和模块导航只调用语义化本地图标", () => {
    ...
    const glyphMap = readSource("../../../features/shared/icon-font.generated.ts");
