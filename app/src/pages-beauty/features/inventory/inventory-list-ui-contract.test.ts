...
describe("库存物品列表定稿契约", () => {
  it("使用独立原生标题、名称搜索、新增和停用范围控制", () => {
    ...
    expect(pages).toMatch(
      /"path": "inventory\/index"[\s\S]*?"navigationBarTitleText": "库存物品"/,
    );
    expect(pages).toMatch(
      /"path": "inventory\/index"[\s\S]*?"navigationBarBackgroundColor": "#FFF8FA"/,
    );
    expect(pages).toMatch(
      /"path": "inventory-detail\/index"[\s\S]*?"navigationBarTitleText": "物品详情"/,
    );
    expect(pages).toContain('"root": "pages-beauty"');
    expect(pages).toContain('"path": "inventory-adjustment/index"');
    expect(pages).toContain('"path": "inventory-profile-edit/index"');
    expect(pages).toMatch(
      /"path": "inventory-create\/index"[\s\S]*?"navigationBarTitleText": "新增库存物品"/,
    );
  ...
  it("库存列表不再追加全局流水，卡片进入独立物品详情", () => {
    ...
    expect(management).toContain("/pages-beauty/inventory-detail/index?inventoryItemId=");
  ...
  it("详情按当前物品展示概览、资料和库存动态", () => {
    ...
    expect(pages).toContain('"path": "inventory-detail/index"');
  ...
  it("预约消耗从物品动态进入对应来源预约", () => {
    const detailPage = readSource("./components/InventoryItemDetailPage.vue");
    const appointmentPage = readSource("../../appointment/index.vue");
    const appointmentManagement = readSource(
      "../appointment/components/AppointmentManagement.vue",
    );

    expect(detailPage).toContain(
      "/pages-beauty/appointment/index?appointmentId=",
    );
  ...
  it("新增使用独立页面，表单没有重复页头并锁定提交中的全部字段", () => {
    ...
    const createRoute = readSource("../../inventory-create/index.vue");
    ...
    expect(management).toContain("/pages-beauty/inventory-create/index");
  ...
  it("补货、盘点修正和资料编辑使用独立页面且不重复页头操作", () => {
    ...
    expect(management).toContain("/pages-beauty/inventory-adjustment/index?");
    expect(detailPage).toContain("/pages-beauty/inventory-adjustment/index?");
    expect(detailPage).toContain("/pages-beauty/inventory-profile-edit/index?");
