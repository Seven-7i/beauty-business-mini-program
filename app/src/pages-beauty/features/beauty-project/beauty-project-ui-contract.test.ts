...
describe("服务项目确认稿界面契约", () => {
  ...
  it("新增和编辑复用独立表单并保护草稿与异步完成", () => {
    ...
    const route = readSource("../../beauty-project-create/index.vue");
    ...
    expect(pages).toContain('"path": "beauty-project-create/index"');
  ...
  it("详情按概览、默认用量和低频操作分层且不引入无意义 Tab", () => {
    const pages = readSource("../../../pages.json");
    const route = readSource("../../beauty-project-detail/index.vue");
    ...
    expect(pages).toContain('"path": "beauty-project-detail/index"');
