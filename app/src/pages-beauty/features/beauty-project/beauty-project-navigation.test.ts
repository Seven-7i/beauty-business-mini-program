...
describe("服务项目独立页面导航", () => {
  it("新增使用基础地址，编辑和详情只附加编码后的稳定项目标识", () => {
    expect(buildBeautyProjectEditorUrl()).toBe(
      "/pages-beauty/beauty-project-create/index",
    );
    expect(buildBeautyProjectEditorUrl("project/深层补水")).toBe(
      "/pages-beauty/beauty-project-create/index?projectId=project%2F%E6%B7%B1%E5%B1%82%E8%A1%A5%E6%B0%B4",
    );
    expect(buildBeautyProjectDetailUrl("project/深层补水")).toBe(
      "/pages-beauty/beauty-project-detail/index?projectId=project%2F%E6%B7%B1%E5%B1%82%E8%A1%A5%E6%B0%B4",
    );
  });
  ...
  it("列表和详情通过聚焦入口打开对应页面", () => {
    ...
    expect(navigateTo).toHaveBeenNthCalledWith(1, {
      url: "/pages-beauty/beauty-project-create/index",
    });
    expect(navigateTo).toHaveBeenNthCalledWith(2, {
      url: "/pages-beauty/beauty-project-create/index?projectId=project-1",
    });
    expect(navigateTo).toHaveBeenNthCalledWith(3, {
      url: "/pages-beauty/beauty-project-detail/index?projectId=project-1",
    });
  });
  ...
  it("有上一页时返回，深链入口则重建到服务项目列表", () => {
    ...
    expect(reLaunch).toHaveBeenCalledWith({
      url: "/pages-beauty/beauty-project/index",
    });
  });
