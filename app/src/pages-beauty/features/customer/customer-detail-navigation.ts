...
/** 顾客列表的稳定页面地址。 */
export const CUSTOMER_LIST_URL = "/pages-beauty/customer/index";

/** 构造从顾客历史预约进入独立预约详情页的地址。 */
export function buildCustomerAppointmentDetailUrl(appointmentId: string): string {
  return `/pages-beauty/appointment-detail/index?appointmentId=${encodeURIComponent(appointmentId)}`;
}

/** 从顾客详情的历史预约卡片进入对应预约详情页。 */
export function openCustomerAppointmentDetail(
  appointmentId: string,
  runtime: Pick<CustomerDetailNavigationRuntime, "navigateTo"> =
    uni as unknown as CustomerDetailNavigationRuntime,
): void {
  runtime.navigateTo({ url: buildCustomerAppointmentDetailUrl(appointmentId) });
}

/** 构造只携带稳定顾客标识的详情页地址。 */
export function buildCustomerDetailUrl(customerId: string): string {
  return `/pages-beauty/customer-detail/index?customerId=${encodeURIComponent(customerId)}`;
}
