...
import {
  ...
  buildCustomerAppointmentDetailUrl,
  completeCustomerDetailDeletion,
  openCustomerAppointmentDetail,
  openCustomerDetail,
...
describe("顾客详情导航", () => {
  it("从历史预约卡片进入对应预约详情页", () => {
    ...
    openCustomerAppointmentDetail("appointment/2026-09", { navigateTo });

    expect(buildCustomerAppointmentDetailUrl("appointment/2026-09")).toBe(
      "/pages-beauty/appointment-detail/index?appointmentId=appointment%2F2026-09",
    );
    expect(navigateTo).toHaveBeenCalledWith({
      url: "/pages-beauty/appointment-detail/index?appointmentId=appointment%2F2026-09",
    });
  });
  ...
  it("使用编码后的稳定顾客标识进入独立详情页", () => {
    ...
    expect(buildCustomerDetailUrl("customer/张女士")).toBe(
      "/pages-beauty/customer-detail/index?customerId=customer%2F%E5%BC%A0%E5%A5%B3%E5%A3%AB",
    );
    expect(navigateTo).toHaveBeenCalledWith({
      url: "/pages-beauty/customer-detail/index?customerId=customer%2F%E5%BC%A0%E5%A5%B3%E5%A3%AB",
    });
  });
  ...
  it("有上一页时返回，深链入口则重建到顾客列表", () => {
    ...
    expect(reLaunch).toHaveBeenCalledWith({ url: "/pages-beauty/customer/index" });
  });
});
