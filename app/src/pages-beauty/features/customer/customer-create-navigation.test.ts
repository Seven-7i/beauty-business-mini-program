...
describe("统一顾客表单页导航", () => {
  it("新增使用基础地址，编辑只附加编码后的稳定顾客标识", () => {
    expect(buildCustomerEditorUrl()).toBe("/pages-beauty/customer-create/index");
    expect(buildCustomerEditorUrl("customer/张女士")).toBe(
      "/pages-beauty/customer-create/index?customerId=customer%2F%E5%BC%A0%E5%A5%B3%E5%A3%AB",
    );
  ...
  it("列表和详情通过同一导航入口打开对应表单模式", () => {
    ...
    expect(navigateTo).toHaveBeenNthCalledWith(1, {
      url: "/pages-beauty/customer-create/index",
    });
    expect(navigateTo).toHaveBeenNthCalledWith(2, {
      url: "/pages-beauty/customer-create/index?customerId=customer-1",
    });
  });
  ...
  it("深链根页面保存后回到对应的详情或列表", () => {
    ...
    expect(runtime.reLaunch).toHaveBeenLastCalledWith({
      url: "/pages-beauty/customer-detail/index?customerId=customer-1",
    });

    completeCustomerEditorNavigation("", runtime, 1);
    expect(runtime.reLaunch).toHaveBeenLastCalledWith({
      url: "/pages-beauty/customer/index",
    });
  });
