...
describe("预约执行定稿契约", () => {
  it("注册列表、新增和详情三条独立路由", () => {
    ...
    expect(pages).toContain('"path": "appointment/index"');
    expect(pages).toContain('"path": "appointment-create/index"');
    expect(pages).toContain('"path": "appointment-detail/index"');
  });
  ...
  it("编辑页更新原生标题，完成与取消弹层就地展示操作错误", () => {
    const route = readSource("../../appointment-create/index.vue");
    const completion = readSource(
      "./components/AppointmentCompletionForm.vue",
    );
  ...
  it("新增预约资料不足时，说明和入口逐项对应缺失的顾客与服务项目", () => {
    ...
    expect(page).toContain('url: "/pages-beauty/customer-create/index"');
    expect(page).toContain('url: "/pages-beauty/beauty-project-create/index"');
  });
  ...
  it("列表完成预约保持定稿的紧凑按钮比例", () => {
    const list = readSource("./components/AppointmentList.vue");

    expect(list).toMatch(
      /\.appointment-card__complete\s*\{[^}]*min-height:\s*68rpx;[^}]*height:\s*70rpx/s,
    );
    expect(list).not.toContain("appointment-card__complete-visual");
  });
