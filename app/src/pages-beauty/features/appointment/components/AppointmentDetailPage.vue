...
const dialogOpen = computed(
  () => Boolean(completingAppointment.value || cancellingAppointment.value),
);
...
function openEdit(): void {
  ...
  uni.navigateTo({
    url: `/pages-beauty/appointment-create/index?appointmentId=${encodeURIComponent(appointment.value.id)}`,
  });
}
...
<template>
  <view class="appointment-detail">
    <view class="appointment-detail__content" :aria-hidden="dialogOpen">
...
.appointment-detail { min-height: 100vh; box-sizing: border-box; padding: 24rpx 28rpx calc(38rpx + env(safe-area-inset-bottom)); background: $surface-page-violet; }
.appointment-detail__state { padding: 80rpx 30rpx; color: #827b88; font-size: 24rpx; text-align: center; }
.detail-hero, .detail-card { margin-bottom: 20rpx; padding: 30rpx; border: 2rpx solid #f0ecf2; border-radius: 22rpx; background: $surface-card; box-shadow: 0 10rpx 28rpx rgba(43, 30, 59, 0.05); }
...
.detail-hero__phone { display: flex; min-height: 54rpx; align-items: center; align-self: flex-start; gap: 10rpx; margin: 10rpx 0 0; padding: 0 18rpx; border: 1rpx solid #e3d8f5; border-radius: 999rpx; background: #f3eefb; color: $accent-strong; font-size: 24rpx; line-height: 1; transition: background-color $press-duration ease, border-color $press-duration ease, transform $press-duration ease; }
...
.detail-row { min-height: 72rpx; align-items: flex-start; justify-content: space-between; gap: 18rpx; padding: 12rpx 0; border-bottom: 2rpx solid $border-soft; color: #5f5966; font-size: 23rpx; }
...
.detail-card--operations button { display: flex; width: 100%; height: 78rpx; align-items: center; gap: 14rpx; padding: 0; border-bottom: 2rpx solid $border-soft; background: transparent; color: $accent-strong; font-size: 24rpx; line-height: 78rpx; text-align: left; }
