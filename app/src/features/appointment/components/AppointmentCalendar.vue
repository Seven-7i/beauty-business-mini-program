...
<template>
  <view class="calendar-panel">
    ...
    <view class="calendar-panel__day-list">
      ...
      <view v-else-if="errorMessage" class="calendar-panel__empty calendar-panel__empty--error">{{ errorMessage }}</view>
      <view v-else-if="!selectedAppointments.length" class="calendar-panel__empty">当天没有预约</view>
...
.calendar-panel__heading { justify-content: space-between; }
...
.calendar-panel__empty--error { border-color: #e4bdbc; color: #97423f; }
.calendar-appointment { width: 100%; min-height: 90rpx; gap: 16rpx; margin-top: 12rpx; padding: 16rpx 18rpx; border: 2rpx solid #e0e5ec; border-radius: 14rpx; background: #fff; text-align: left; }
.calendar-appointment__time { color: #31549e; font-size: 23rpx; font-weight: 700; }
.calendar-appointment__copy { display: flex; min-width: 0; flex: 1; flex-direction: column; color: #2b374d; font-size: 22rpx; }
.calendar-appointment__copy text:last-child { overflow: hidden; margin-top: 5rpx; color: #7b8595; font-size: 18rpx; text-overflow: ellipsis; white-space: nowrap; }
.calendar-appointment__status { color: #68758a; font-size: 18rpx; }
</style>
