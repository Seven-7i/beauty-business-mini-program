...
describe("库存管理用例", () => {
  ...
  it("编辑手工补货记录时原子更新库存与记录", async () => {
    const memory = createMemoryRepository({
      ...createEmptyData(),
      inventoryItems: [createItem({ currentQuantity: "12" })],
      inventoryMovements: [
        {
          id: "movement-1",
          inventoryItemId: "item-1",
    ...
    await service.rewriteManualMovement({
    ...
    expect(memory.readData().inventoryItems[0]?.currentQuantity).toBe("13.5");
    expect(memory.readData().inventoryMovements[1]).toMatchObject({
      deltaQuantity: "3.5",
      afterQuantity: "13.5",
      note: "更正数量",
    });
